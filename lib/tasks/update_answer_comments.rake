namespace :update_answer_comments do
  ...
  task :update_answer_comments => :environment do
    ...
    ans.comments.delete_all
		users = User.all
		ans.comments.create({user_id:users[2].id,body:@commentFirst})
		ans.comments.create({user_id:users[3].id,body:@commentSecond})
		ans.comments.create({user_id:users[4].id,body:@commentThree})
	end
  end
